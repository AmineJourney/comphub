...
import { CreateDepartmentDialog } from "./DepartmentDialogs";
import { useI18n } from "@/hooks/useI18n";

export function DepartmentTree() {
  const { t } = useI18n();
  const canCreateDepartments = useAuthStore((state) => state.hasPermission("create_any"));
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("departments.tree.hierarchy")}</CardTitle>
        </CardHeader>
        <CardContent>
          {tree && tree.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">{t("departments.noDepartments")}</p>
              {canCreateDepartments && (
                <Button onClick={() => setShowCreateDialog(true)}>
                  <Plus className="h-4 w-4 mr-2" />
                  {t("departments.tree.createFirst")}
                </Button>
              )}
            </div>
          )}
  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        <div className="flex-shrink-0">
          <Badge variant="outline" className="bg-white">
            {t("departments.tree.level", { level: level + 1 })}
          </Badge>
        </div>
      </div>
