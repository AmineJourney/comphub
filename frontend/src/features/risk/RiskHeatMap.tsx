...
export function RiskHeatMap() {
  ...
  if (!heatMapData) {
    return <div>{t("risk.heatMapNoData")}</div>;
  }
  ...
  const renderHeatMap = (grid: any[][], title: string) => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>{t("risk.heatMapDescription")}</CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="mt-6 flex items-center justify-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-700">{getRiskLevelLabel("low", language)}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 bg-yellow-500 rounded" />
            <span className="text-sm text-gray-700">{getRiskLevelLabel("medium", language)}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 bg-orange-500 rounded" />
            <span className="text-sm text-gray-700">{getRiskLevelLabel("high", language)}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 bg-red-500 rounded" />
            <span className="text-sm text-gray-700">{getRiskLevelLabel("critical", language)}</span>
          </div>
        </div>
      </CardContent>
    </Card>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">{t("risk.heatMapTitle")}</h1>
            <p className="text-gray-600 mt-1">
              {t("risk.heatMapSubtitle")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {renderHeatMap(inherentGrid, t("risk.inherentRisk"))}
        {renderHeatMap(residualGrid, t("risk.heatMapResidualAfterControls"))}
      </div>

      {/* Summary */}
      <Card>
        <CardHeader>
          <CardTitle>{t("risk.heatMapSummary")}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div>
              <p className="text-sm text-gray-600">{t("risk.heatMapRiskReduction")}</p>
            ...
            <div>
              <p className="text-sm text-gray-600">{t("risk.avgInherentScore")}</p>
            ...
            <div>
              <p className="text-sm text-gray-600">{t("risk.avgResidualScore")}</p>
              <p className="text-2xl font-bold text-green-600 mt-1">
                {residual.length > 0
                  ? (
                      residual.reduce(
