...
export function FrameworkComplianceDetail() {
  ...
  const tabs = [
    { id: "overview", label: t("compliance.detail.overview"), icon: BarChart2 },
    { id: "gaps", label: t("compliance.reports.types.gapAnalysis"), icon: AlertCircle },
    { id: "requirements", label: t("library.requirements"), icon: CheckCircle },
    { id: "recommendations", label: t("compliance.detail.recommendations"), icon: Target },
  ];
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-bold text-gray-900">
                {result?.framework_code || t("common.framework")}
              </h1>
              {result && (
                <Badge
                  className={
      ...
      {!result ? (
        <Card>
          <CardContent className="text-center py-16">
            ...
            <p className="text-gray-500 text-lg">{t("compliance.detail.noResults")}</p>
            <p className="text-gray-400 mt-2">
              {t("compliance.detail.runCalculation")}
            </p>
            <Button
              className="mt-6"
              onClick={() => calculateMutation.mutate()}
            ...
            >
              ...
              {calculateMutation.isPending
                ...
                : t("compliance.detail.calculateNow")}
            </Button>
          </CardContent>
        </Card>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
            ...
            <Card>
              <CardContent className="p-6">
                <p className="text-sm font-medium text-gray-600">{t("compliance.coverage")}</p>
                <p className="text-3xl font-bold text-gray-900 mt-2">
            ...
            <Card>
              <CardContent className="p-6">
                <p className="text-sm font-medium text-gray-600">
                  {t("compliance.detail.controlsActive")}
                </p>
                <p className="text-3xl font-bold text-gray-900 mt-2">
                  {result.controls_operational}
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  {t("compliance.detail.ofTotal", {
            ...
            <Card>
              <CardContent className="p-6">
                <p className="text-sm font-medium text-gray-600">{t("compliance.openGaps")}</p>
                <p className="text-3xl font-bold text-gray-900 mt-2">
                  {result.gap_count}
                </p>
                {result.gap_count > 0 && (
          ...
          {activeTab === "overview" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  <CardHeader>
                    <CardTitle>{t("compliance.detail.requirementsBreakdown")}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {[
                      {
                        label: t("compliance.detail.compliant"),
                        value: result.requirements_compliant,
                        color: "bg-green-500",
                      },
                      {
                        label: t("compliance.detail.partiallyCompliant"),
                        value: result.requirements_partial,
                        color: "bg-yellow-500",
                      },
                      {
                        label: t("compliance.detail.nonCompliant"),
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>{t("compliance.detail.controlsStatus")}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {[
                      {
                        label: t("controls.statusLabels.operational"),
                        value: result.controls_operational,
                        color: "bg-green-500",
                      },
                      {
                        label: t("controls.statusLabels.implemented"),
                        value: result.controls_implemented,
                        color: "bg-blue-500",
                      },
                      {
                        label: t("controls.statusLabels.in_progress"),
                        value: result.controls_in_progress,
                        color: "bg-yellow-500",
                      },
                      {
                        label: t("controls.statusLabels.not_started"),
              ...
              {trends && trends.length > 0 && (
                <Card>
                  <CardHeader>
                    <CardTitle>{t("compliance.detail.trend")}</CardTitle>
                    <CardDescription>{t("compliance.detail.scoreOverTime")}</CardDescription>
                  </CardHeader>
                  <CardContent>
...
function RequirementsBreakdown({ result }: { result: ComplianceResult }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("compliance.detail.requirementsDetail")}</CardTitle>
        <CardDescription>
          {t("compliance.detail.requirementsDetailDescription", {
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {t(`compliance.detail.requirementStatuses.${req.status}`)}
                    </Badge>
                  </div>
