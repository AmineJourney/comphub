...
class CompanyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action in ('update', 'partial_update', 'destroy'):
            ...
            if tenant and membership and membership.role in ('owner', 'admin'):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if request.tenant != instance:
            return Response(
                {'error': 'You can only manage your current company context.'},
    ...
    def get_queryset(self):
        ...
        if self.action in ('update', 'partial_update', 'destroy', 'retrieve'):
            if (
                ...
                and self.request.tenant
                and hasattr(self.request, 'membership')
                and self.request.membership
                and self.request.membership.role in ('owner', 'admin')
            ):
