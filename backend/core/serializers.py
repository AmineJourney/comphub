...
class InvitationCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_role(self, value):
        # Only owners can invite owners; admins may invite admins and below.
        request = self.context.get('request')
        if request and hasattr(request, 'membership'):
            requester_role = request.membership.role
            if value == 'owner' and requester_role != 'owner':
                raise serializers.ValidationError(
                    "Only owners can invite users with the owner role."
                )
            if value == 'admin' and requester_role not in ('owner', 'admin'):
                raise serializers.ValidationError(
