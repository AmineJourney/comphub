...
class EvidenceViewSet(viewsets.ModelViewSet):
    ...
    def file_content(self, request, pk=None):
        """Stream evidence content for preview/viewing."""
        ...
        if evidence.file_content:
            response = FileResponse(
                BytesIO(evidence.get_file_bytes()),
                content_type=evidence.file_type or 'application/octet-stream'
            )
            response['Content-Disposition'] = f'inline; filename="{evidence.get_serving_filename()}"'
            return response

        if evidence.file:
            return FileResponse(
                evidence.file.open('rb'),
                filename=evidence.get_serving_filename()
            )
