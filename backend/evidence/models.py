...
from core.encryption import decrypt_for_company, encrypt_for_company
...
class Evidence(TenantMixin, TimeStampedModel, SoftDeleteModel):
    ...
    file_content = models.BinaryField(
        ...
        blank=True,
        editable=False,
        help_text='Binary file content stored directly in the database'
    )
    file_content_encrypted = models.BooleanField(
        default=False,
        editable=False,
    ...
    def save(self, *args, **kwargs):
        ...
        if self.file_content:
            if self.original_filename and not self.file_type:
                guessed_type, _ = mimetypes.guess_type(self.original_filename)
                self.file_type = guessed_type or ''
            if self.file_content and not self.file_hash:
                self.file_hash = hashlib.sha256(self.get_file_bytes()).hexdigest()
        elif self.file:
            # Set file size
    ...
    def calculate_file_hash(self):
        ...
        if self.file_content:
            return hashlib.sha256(self.get_file_bytes()).hexdigest()
    ...
    def get_serving_filename(self):
        """Get the best available filename for downloads/previews."""
        if self.original_filename:
    ...
    def get_file_bytes(self):
        """Return the plaintext file bytes regardless of storage format."""
        if not self.file_content:
            return b''

        file_bytes = bytes(self.file_content)
        if not self.file_content_encrypted:
            return file_bytes

        return decrypt_for_company(self.company_id, file_bytes)
    ...
    def delete_file(self):
        """Delete stored file content and any legacy physical file."""
        if self.file_content:
            self.file_content = None
            self.file_content_encrypted = False
        if self.file:
    ...
    def store_uploaded_file(self, uploaded_file):
        """Persist uploaded content directly in the database."""
        if not uploaded_file:
            return

        self.original_filename = uploaded_file.name
        self.file_size = uploaded_file.size
        self.file_type = getattr(uploaded_file, 'content_type', '') or (
            mimetypes.guess_type(uploaded_file.name)[0] or ''
        )

        file_bytes = uploaded_file.read()
        self.file_content = encrypt_for_company(self.company_id, file_bytes)
        self.file_content_encrypted = True
