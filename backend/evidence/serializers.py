...
from library.scoping import limit_framework_codes, limit_framework_coverage, limit_requirement_mappings


class EvidenceSerializer(serializers.ModelSerializer):
    file = serializers.FileField(write_only=True, required=False)
    file_name = serializers.CharField(source='get_serving_filename', read_only=True)
    ...
    class Meta:
        model = Evidence
        fields = [
            'id', 'company', 'name', 'description', 'file', 'file_name', 'file_url',
    ...
    def get_file_url(self, obj):
        ...
        if request and (obj.file_content or obj.file):
            return reverse('evidence-file-content', kwargs={'pk': obj.pk}, request=request)
        return None
    ...
    def create(self, validated_data):
        ...
        if uploaded_file:
            evidence.store_uploaded_file(uploaded_file)
        evidence.save()
        return evidence
    ...
    def update(self, instance, validated_data):
        ...
        if uploaded_file:
            instance.store_uploaded_file(uploaded_file)
        instance.save()
        return instance
...
class AppliedControlEvidenceSerializer(serializers.ModelSerializer):
    ...
    # ── KEY FIELD ──────────────────────────────────────────────────────────────
    # Returns the distinct framework codes that this evidence supports, including
    # frameworks reached through the linked unified control when present.
    ...
    def get_frameworks(self, obj):
        ...
        if obj.applied_control.unified_control_id:
            direct_frameworks.update(
                limit_framework_coverage(
                    obj.applied_control.unified_control.get_framework_coverage()
                ).keys()
            )

        return limit_framework_codes(direct_frameworks)
