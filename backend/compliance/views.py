...
from library.scoping import limit_frameworks, framework_scope_q
    ...
    def get_queryset(self):
        ...
        if hasattr(self.request, 'tenant'):
            return ComplianceGap.objects.for_company(
                ...
                framework_scope_q('requirement__framework__code')
            ).select_related(
...
class FrameworkAdoptionViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'])
    def carryover_preview(self, request):
        framework_id = request.query_params.get('framework')
        if not framework_id:
            return Response(
        ...
        preview = ComplianceCalculationService.get_framework_adoption_preview(
            company=request.tenant,
            framework=framework,
        )
    ...
    def get_queryset(self):
        ...
        if hasattr(self.request, 'tenant'):
            return ComplianceReport.objects.for_company(
                ...
                framework_scope_q('framework__code') | Q(framework__isnull=True)
