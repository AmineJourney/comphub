...
from .scoping import (
    limit_frameworks,
    limit_loaded_libraries,
    limit_requirements,
    limit_stored_libraries,
)
...
class LoadedLibraryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return limit_loaded_libraries(
            super().get_queryset().filter(is_deleted=False)
        ).select_related('stored_library')

    @action(detail=True, methods=['get'])
    def frameworks(self, request, pk=None):
        """Get all frameworks in this library version"""
        loaded_library = self.get_object()
        frameworks = limit_frameworks(
            loaded_library.frameworks.filter(is_deleted=False)
        )
        serializer = FrameworkSerializer(frameworks, many=True, context={'request': request})
...
class FrameworkViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = limit_frameworks(super().get_queryset())
    ...
    def requirements(self, request, pk=None):
        ...
        if request.query_params.get('tree') == 'true':
            ...
            serializer = RequirementTreeSerializer(roots, many=True, context={'request': request})
        else:
            serializer = RequirementListSerializer(requirements, many=True, context={'request': request})
    ...
    def requirements_tree(self, request, pk=None):
        ...
        framework = self.get_object()
        roots = framework.requirements.filter(
            parent__isnull=True,
            is_deleted=False,
        ).order_by('sort_order', 'code')
        serializer = RequirementTreeSerializer(
            roots,
            many=True,
...
class RequirementViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = limit_requirements(super().get_queryset())
        qs = qs.filter(
    ...
    def children(self, request, pk=None):
        ...
        serializer = self.get_serializer(children, many=True, context={'request': request})
        return Response(serializer.data)
    ...
    def roots(self, request):
        ...
        serializer = self.get_serializer(roots, many=True, context={'request': request})
        return Response(serializer.data)
