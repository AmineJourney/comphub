...
class DepartmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'manager', 'manager_email', 'full_path', 'children_count', 'company',
    ...
    def validate(self, attrs):
        ...
        company_obj = attrs.get('company') or getattr(self.instance, 'company', None)
        
        # Validate parent belongs to same company
        parent = attrs.get('parent')
        if parent and parent.company != company_obj:
            raise serializers.ValidationError({
                'parent': 'Parent department must belong to the same company'
            })

        manager = attrs.get('manager')
        if manager and company_obj:
            is_member = manager.memberships.filter(
                company=company_obj,
                is_deleted=False,
                is_active=True,
            ).exists()
            if not is_member:
                raise serializers.ValidationError({
                    'manager': 'Manager must be an active member of the same company'
                })
...
class DepartmentTreeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Department
        fields = ['id', 'name', 'description', 'code', 'manager', 'manager_email', 'children']
